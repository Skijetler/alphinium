...
// QuerySubcategories chains the current query on the "subcategories" edge.
func (cq *CategoryQuery) QuerySubcategories() *SubcategoryQuery {
	query := &SubcategoryQuery{config: cq.config}
...
// the "subcategories" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CategoryQuery) WithSubcategories(opts ...func(*SubcategoryQuery)) *CategoryQuery {
	query := &SubcategoryQuery{config: cq.config}
...
func (cq *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := cq.withSubcategories; query != nil {
		if err := cq.loadSubcategories(ctx, query, nodes,
			func(n *Category) { n.Edges.Subcategories = []*Subcategory{} },
			func(n *Category, e *Subcategory) { n.Edges.Subcategories = append(n.Edges.Subcategories, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CategoryQuery) loadSubcategories(ctx context.Context, query *SubcategoryQuery, nodes []*Category, init func(*Category), assign func(*Category, *Subcategory)) error {
	...
	query.Where(predicate.Subcategory(func(s *sql.Selector) {
		s.Where(sql.InValues(category.SubcategoriesColumn, fks...))
	}))
