...
type UserQuery struct {
	...
	withMetadata    *UserMetadataQuery
	withPosts       *PostQuery
...
func (uq *UserQuery) QueryMetadata() *UserMetadataQuery {
	query := &UserMetadataQuery{config: uq.config}
...
func (uq *UserQuery) QueryAttachments() *AttachmentQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withAttachments: uq.withAttachments.Clone(),
...
func (uq *UserQuery) WithAttachments(opts ...func(*AttachmentQuery)) *UserQuery {
	query := &AttachmentQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withAttachments = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withMetadata; query != nil {
		if err := uq.loadMetadata(ctx, query, nodes, nil,
			func(n *User, e *UserMetadata) { n.Edges.Metadata = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := uq.withAttachments; query != nil {
		if err := uq.loadAttachments(ctx, query, nodes,
...
func (uq *UserQuery) loadMetadata(ctx context.Context, query *UserMetadataQuery, nodes []*User, init func(*User), assign func(*User, *UserMetadata)) error {
	...
	query.Where(predicate.UserMetadata(func(s *sql.Selector) {
...
func (uq *UserQuery) loadAttachments(ctx context.Context, query *AttachmentQuery, nodes []*User, init func(*User), assign func(*User, *Attachment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Attachment(func(s *sql.Selector) {
		s.Where(sql.InValues(user.AttachmentsColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
