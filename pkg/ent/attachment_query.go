...
func (aq *AttachmentQuery) QueryPost() *PostQuery {
...
func (aq *AttachmentQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (aq *AttachmentQuery) FirstIDX(ctx context.Context) uint64 {
...
func (aq *AttachmentQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (aq *AttachmentQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (aq *AttachmentQuery) IDs(ctx context.Context) ([]uint64, error) {
	var ids []uint64
...
func (aq *AttachmentQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := aq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (aq *AttachmentQuery) WithPost(opts ...func(*PostQuery)) *AttachmentQuery {
	query := &PostQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AttachmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attachment, error) {
	...
	if query := aq.withPost; query != nil {
		if err := aq.loadPost(ctx, query, nodes, nil,
			func(n *Attachment, e *Post) { n.Edges.Post = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AttachmentQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *User)) error {
	...
	nodeids := make(map[uint64][]*Attachment)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (aq *AttachmentQuery) loadPost(ctx context.Context, query *PostQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *Post)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Attachment)
	for i := range nodes {
		fk := nodes[i].PostID
